...
description: |
  ...
  ⚠️ IMPORTANT: This agent runs in isolated context. NO MCP tools available.
  ...
  Workflow:
  1. Caller (main context) captures URL → saves snapshot locally
  2. This agent processes snapshot → extracts links → updates queue
  3. Caller reads queue → captures next URL → loops
...
```
{output_dir}/
├── config.json          # Crawl configuration (read-only after init)
├── queue.json           # URL queue with priorities
├── visited.json         # Visited URL list
├── pages/               # Page content (NEVER read these!)
│   ├── 001_*.md
│   ├── 002_*.md
│   └── ...
├── links/               # Discovered links per page
│   ├── 001.json
│   ├── 002.json
│   └── ...
├── INDEX.md             # Summary index (generated by summarizer)
└── REPORT.md            # Topic report (generated by summarizer)
```
...
│      Write config.json                                                     │
│      Write queue.json with entry URL                                       │
│      Write visited.json (empty)                                            │
│                                                                             │
│  Step 2: Process Loop                                                      │
│  ────────────────────                                                       │
│    while queue not empty AND processed < max_pages:                        │
│                                                                             │
│      a. Read queue.json                                                    │
│      b. Sort by priority (descending)                                      │
│      c. Pop highest priority URL                                           │
│      d. Check if already visited → skip if yes                             │
│      e. Check depth limit → skip if exceeded                               │
│                                                                             │
│      f. Call extractor agent:                                              │
│         Task(                                                              │
│           subagent_type: "knowledge-etl:extractor",                        │
│           prompt: "Extract URL with link discovery..."                     │
│         )                                                                  │
│         → Agent writes: pages/{id}_{slug}.md, links/{id}.json              │
│                                                                             │
│      g. Read links/{id}.json (small file, safe)                            │
│      h. For each link:                                                     │
│         - Skip if already visited                                          │
│         - Skip if different domain (if same_domain=true)                   │
│         - Skip if filtered URL pattern                                     │
│         - Calculate priority = link_score + parent_relevance_bonus         │
│         - Add to queue.json                                                │
│                                                                             │
│      i. Update visited.json with current URL                               │
│      j. Update queue.json                                                  │
│                                                                             │
│  Step 3: Summarize                                                         │
│  ─────────────────                                                          │
│    Call crawler-summarizer agent:                                          │
│    Task(                                                                   │
│      subagent_type: "knowledge-etl:crawler-summarizer",                    │
│      prompt: "Generate INDEX.md and REPORT.md..."                          │
│    )                                                                       │
│                                                                             │
│  Step 4: Transform (if --pipe specified)                                   │
│  ────────────────────────────────────────                                   │
│    if config.pipe:                                                         │
│      Call output-transformer agent:                                        │
│      Task(                                                                 │
...
│  ──────────────────────                                                     │
│    Read and return brief completion message                                │
│    (DO NOT read full content files)                                        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
if parent_relevance >= 8 (high):
  → All child links get +10 bonus (prioritize this branch)
  → Deep-first exploration of this knowledge path

if parent_relevance >= 5 (medium):
  → Child links get +5 bonus
  → Normal queue order

if parent_relevance < 5 (low):
  → Child links get +0 bonus
  → Low priority, may not be processed if max_pages reached
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║  ⚠️ MCP TOOLS NOT AVAILABLE IN THIS AGENT                                 ║
║                                                                            ║
║  The caller (extract.md in main context) captures URLs before calling.    ║
║  This agent only processes LOCAL snapshot files.                          ║
╚═══════════════════════════════════════════════════════════════════════════╝
