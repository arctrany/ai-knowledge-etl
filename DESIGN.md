...
## ⛔ 关键配置：防止 "Prompt is too long"

```
╔═══════════════════════════════════════════════════════════════════════════╗
║  🚨 MCP 配置必须包含 --image-responses omit 🚨                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  默认情况下，browser_take_screenshot 会将图片嵌入到对话上下文中，         ║
║  这会导致 "Prompt is too long" 错误。                                     ║
║                                                                           ║
║  解决方案：在 .mcp.json 中配置 --image-responses omit                     ║
║                                                                           ║
║  配置示例 (.mcp.json):                                                    ║
║  {                                                                        ║
║    "playwright": {                                                        ║
...
║        "--user-data-dir", "~/.playwright-mcp",                           ║
║        "--image-responses", "omit",        ← 关键配置！                   ║
║        "--viewport-size", "1280,720"                                      ║
║      ]                                                                    ║
║    }                                                                      ║
║  }                                                                        ║
║                                                                           ║
║  效果：截图只保存到文件，不嵌入上下文，由子代理读取处理                   ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
```
...
2. **主上下文提取并下载图片**: 使用 browser_evaluate 获取图片URL，curl 下载（v3.0 新增）
3. **子代理处理本地文件**: 读取保存的文件（快照、截图、已下载图片）进行内容处理
4. **分工明确**: 主上下文 = 网络交互 + 图片下载，子代理 = 内容处理（隔离上下文防溢出）
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  PHASE 1: 主上下文 - 页面捕获 + 图片下载                                  │
│  ──────────────────────────────────────────                              │
│  1. browser_navigate → 加载页面                                          │
│  2. browser_press_key("End") → 触发懒加载                               │
│  3. browser_snapshot → 保存文本快照                                      │
│  4. browser_evaluate → 提取图片URL (过滤装饰图)                          │
│  5. browser_evaluate → 获取cookies                                       │
│  6. curl + cookies → 并行下载图片到本地                                  │
│  7. compress-image.sh → 压缩大图片 (<300KB)                              │
│                                                                          │
│  输出文件:                                                               │
│  ├── .playwright-mcp/snapshot.md     (文本快照)                          │
│  ├── .playwright-mcp/screenshot.png  (整页截图，备用)                    │
│  ├── .playwright-mcp/images.json     (图片元数据)                        │
│  └── .playwright-mcp/img_*.jpg       (已下载并压缩的图片)                │
│                                                                          │
│  PHASE 2: 子代理 - 内容处理                                               │
│  ──────────────────────────────                                          │
│  1. 读取 snapshot.md (分块处理大文件)                                    │
│  2. 读取 images.json (图片元数据)                                        │
│  3. 逐个读取 img_*.jpg，生成文字描述                                     │
│  4. 组合文本 + 图片描述 → 纯 Markdown                                    │
│                                                                          │
│  关键约束:                                                               │
│  ├── 每次只读取一张图片 (防止上下文溢出)                                 │
│  ├── 读取后立即描述并释放                                                │
...
**图片筛选规则**:
- 尺寸 ≥ 100x100 像素
- 跳过: icon, logo, avatar, emoji, button, arrow, loading 等装饰图
- 最多下载5张图片
- 单张最大5MB，下载后压缩到 <300KB

### 已知限制 (v3.0.1)

**钉钉文档图片认证问题**:
- 钉钉文档 (alidocs.dingtalk.com) 的图片API使用复杂认证机制
- 仅传递 document.cookie 不足以获取图片内容
- 当前行为：提取图片元数据（尺寸、alt文本、URL），但无法下载实际图片内容
- 降级方案：使用 images.json 中的元数据生成占位描述

**解决方案** (v3.1.0 - 修正方案):

```
╔═══════════════════════════════════════════════════════════════════════════╗
║  ⛔ 重要更新: 禁止在主上下文使用 browser_take_screenshot ⛔              ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  browser_take_screenshot 会将图片嵌入对话上下文！                        ║
║  即使只截取一张图片，也可能导致 "Prompt is too long" 错误                ║
║                                                                           ║
║  ❌ 已废弃: 点击浮层后循环截图方案                                       ║
║  ✅ 正确做法: browser_evaluate 提取 URL + curl 下载                      ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
```

**推荐方案: browser_evaluate + curl 下载**
```
1. browser_evaluate 提取页面中图片的 src URL
2. 保存到 images.json
3. 使用 curl 批量下载图片（零上下文影响）
4. 子代理处理本地图片文件（隔离上下文）
...
# Step 2: curl 下载（无上下文影响）
for img in images.json:
  curl -o "img_{index}.jpg" "{src}"

# Step 3: 子代理处理
Task(subagent: "extractor", prompt: "处理本地图片...")
```

**优势**:
- 零上下文增长
- 可处理任意数量图片
- 不会触发 "Prompt is too long"

**降级选项**:
- 如果图片需要认证无法下载，使用 images.json 元数据生成文字描述占位

### Transform 阶段防溢出机制 (v3.3.0 新增)

```
╔═══════════════════════════════════════════════════════════════════════════╗
║  🛡️ 分层摘要架构 - 防止 Transform 阶段溢出                                ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  问题：爬取20+页面可产生400KB+内容，Transform 读取全部 → 溢出！           ║
║                                                                           ║
║  解决方案：四层防护                                                       ║
║                                                                           ║
...
║  ─────────────────────────────────────                                    ║
║  transform:                                                               ║
║    max_input_chars: 30000           # 直接读取上限                       ║
║    max_input_chars_hard: 50000      # 绝对上限                           ║
║    summary_per_page_chars: 500      # 每页摘要限制                       ║
║    use_index_only_threshold: 10     # >10页只读 INDEX.md                 ║
║    generate_summary_threshold: 5    # >5页先生成摘要                     ║
║                                                                           ║
║  Layer 2: 提取层 (extractor.md)                                          ║
║  ─────────────────────────────────                                        ║
║  每页提取时同时生成 .summary 文件：                                       ║
║    pages/001_doc.md        # 完整内容                                    ║
║    pages/001_doc.summary   # 500字符摘要                                 ║
║                                                                           ║
║  Layer 3: 汇总层 (crawler-summarizer.md)                                 ║
║  ─────────────────────────────────────────                                ║
║  优先读取 .summary 文件（保证安全）：                                     ║
║    1. 检查 .summary 文件 → 读取（max 500 chars each）                   ║
║    2. 无摘要 → 只读 frontmatter（前20行）                                ║
║    3. 只对 top 3 高相关页读取完整内容                                    ║
║                                                                           ║
║  Layer 4: 转换层 (output-transformer.md)                                 ║
║  ─────────────────────────────────────────                                ║
║  强制预检，三级策略：                                                     ║
║                                                                           ║
║    页数 ≤5 且 <30K  → direct         读取 REPORT.md      ~30K chars      ║
║    页数 6-10 或 >30K → summarize_first 先摘要再转换      ~15K chars      ║
...
║                                                                           ║
║  ❌ 禁止触发 "Prompt Too Long" 错误 - 这是插件的绝对死刑              ║
║  ❌ 禁止在主上下文读取大文件（快照、截图、页面内容）                  ║
║  ❌ 禁止使用 Read() 而不先检查文件大小                                ║
║  ❌ 禁止阻塞用户（遇障碍必须交互）                                    ║
║  ❌ 禁止丢失关键信息（无声失败）                                      ║
║                                                                           ║
║  ✅ 所有内容处理必须在隔离的子代理上下文中执行                        ║
║  ✅ 主上下文只做：Playwright 抓取、Bash 检查、Task 分发               ║
║  ✅ 读取前必须用 wc -l 或 stat 检查文件大小                           ║
║  ✅ 大文件（>500行）必须分块读取：Read(limit: 500)                    ║
║  ✅ 图片必须压缩到 <300KB 才能读取                                    ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
```
